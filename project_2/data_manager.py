...
def get_index_data(index, start_date, end_date):
    if index == 'Nasdaq 100':
        query = f"SELECT * FROM nasdaq100_index where date BETWEEN '{start_date}' AND '{end_date}'"
    elif index == 'S&P 500':
        query = f"SELECT * FROM sp500_index where date BETWEEN '{start_date}' AND '{end_date}'"
    data = pd.read_sql(query, conn)
    data['date'] = pd.to_datetime(data.date)
    data.dropna(inplace=True)
    return data
